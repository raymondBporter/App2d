...
public sealed class GreenDinosaur2DTests
{
    ...
    public void LoadsWalkCycleAndTracksPatrolState()
    {
        ...
        var dinosaur = new GreenDinosaur2D(enemy);

        using var presentation = new EnemyPresentation2D(scene, textures,
            TraversalMetricsLoader2D.Load(TestAssetPath.Root), new SilentSounds());
        dinosaur.Update(0.2f, Vector2.Zero);
        Assert.Empty(scene);
        presentation.Update([dinosaur.CaptureState()], [], 0.2f, 1);
        ...
        presentation.Update([dinosaur.CaptureState()], [], 0f, 2);
