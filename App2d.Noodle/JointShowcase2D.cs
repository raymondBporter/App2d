...
internal sealed class JointShowcase2D
{
    ...
    private readonly RevoluteJoint2D _revolute = new(78f, -0.8f, 0.8f);
    private readonly PrismaticJoint2D _prismatic = new(new Vector2(1f, 0.18f), -72f, 72f);
    private readonly DistanceJoint2D _distance = new(68f, 68f);
    ...
    private void DrawRevolute(Renderer2D renderer, Camera2D camera, double time, Vector2 anchor)
    {
        ...
        var constrained = _revolute.Constrain(anchor, requested);
        DrawRequestedAndConstrained(renderer, anchor, requested, constrained);
        DrawLabel(renderer, camera, _revolute, anchor + new Vector2(-86f, 67f));
    }
    ...
    private void DrawPrismatic(Renderer2D renderer, Camera2D camera, double time, Vector2 anchor)
    {
        ...
        var constrained = _prismatic.Constrain(anchor, requested);
        ...
        DrawLabel(renderer, camera, _prismatic, anchor + new Vector2(-86f, 64f));
    }
    ...
    private void DrawDistance(Renderer2D renderer, Camera2D camera, double time, Vector2 anchor)
    {
        ...
        var constrained = _distance.Constrain(anchor, requested);
        DrawRequestedAndConstrained(renderer, anchor, requested, constrained);
        DrawLabel(renderer, camera, _distance, anchor + new Vector2(-86f, 93f));
    }
    ...
    private static void DrawLabel(Renderer2D renderer, Camera2D camera, IJoint2D joint, Vector2 worldPosition)
    {
        var screenPosition = camera.WorldToDevice(worldPosition);
        renderer.DrawScreenText(joint.Name, screenPosition, XnaColor.White);
        renderer.DrawScreenText(joint.Constraint, screenPosition + new Vector2(0f, 22f), LabelColor);
    }
}
