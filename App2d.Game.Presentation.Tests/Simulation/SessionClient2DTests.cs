...
public sealed class SessionClient2DTests
{
    ...
    public void AttachAtNonzeroTickAndSendInputsWithoutWaitingForResponses()
    {
        ...
        var client = new SessionClient2D(snapshot);
        ...
        var frames = inputs.Select(game.Session.Advance).ToArray();
    ...
    public void InvalidDeliveryCannotPartiallyChangeClientClocksOrObservation()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => client.Apply(frame with { LastInputSequence = 0 }));
    ...
    {
        using var game = new Fixture();
        var client = new SessionClient2D(game.Session.CaptureState());
