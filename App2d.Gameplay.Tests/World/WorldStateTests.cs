...
public sealed class WorldStateTests
{
    ...
    public void TerrainSnapshotsKeepOldTilesAndHaloWhileDirtyChunksGetNewRevisions()
    {
        ...
        var original = level.CaptureContent();
        ...
        Assert.Same(original, level.CaptureContent()); // Unchanged content is the same instance.
        map.SetTileKind(32, 4, TileKind2D.Solid);
        level.FlushDirtyChunks();
        var edited = level.CaptureContent();
        ...
        Assert.DoesNotContain(level.CaptureContent().Terrain, c => c.Chunk == left.Chunk);
        level.UpdateStreaming(level.SpawnPoint);
        Assert.NotEqual(changedLeft.Revision, Assert.Single(level.CaptureContent().Terrain, c => c.Chunk == left.Chunk).Revision);
    }
    ...
    public void ProductionLevelRunsInSessionWithoutGraphicsAndReturnsCheckpointAndPlatformState()
    {
        ...
        var player = new Person2D(EntityId2D.Create(), physics.CollisionSystem, physics, metrics, spawn, 2, 1, CombatFaction2D.Player, tileMap: map);
        registry.Register(player);
        var arsenal = new PersonArsenal2D(new EntityIdAllocator2D(), player.Body, metrics.GunMuzzleOffset, physics.CollisionSystem, 1, 4, CombatFaction2D.Player, combat);
        ...
        var initialContent = session.CaptureContent();
        ...
        Assert.Equal(things[2].Position, entered.Content.GoalPosition);
        Assert.Same(initialContent, entered.Content); // Nothing streamed or changed, so content is shared.
        ...
        Assert.NotEmpty(initialContent.Terrain); // Disposal cannot invalidate an earlier observation.
