...
internal sealed class StudioDocument
{
    ...
    public StudioDocument(string path) : this(PointLibrary.Load(path)) { }
    ...
    public void RecordEdit(CharacterAppearance before, bool active)
    {
        var current = Capture();
    ...
    private string Capture() => JsonSerializer.Serialize(new Snapshot(Appearance, Entity, Bindings), JsonOptions);
    private void Restore(string state)
    {
        var snapshot = JsonSerializer.Deserialize<Snapshot>(state, JsonOptions)!;
    ...
    public void ResetAppearance() { Appearance = Defaults(Library); if (Entity is not null) Entity.Appearance = Appearance; RecordEdit(Appearance, false); }
    public void Undo() { RecordEdit(Appearance, false); if (_undo.TryPop(out var previous)) { _redo.Push(Capture()); Restore(previous); } }
    public void Redo() { if (_redo.TryPop(out var next)) { _undo.Push(Capture()); Restore(next); } }
    public void Bind(string role, string clip) { if (!Library.Clips.ContainsKey(clip)) throw new InvalidDataException("Unknown binding clip."); Bindings[role] = clip; RecordEdit(Appearance, false); }
    ...
    {
        RecordEdit(Appearance, false);
        if (Entity is not null) { Entity.Validate(Library); Entity.Save(path); FilePath = path; Dirty = false; return; }
        Appearance.Validate();
        var content = JsonSerializer.Serialize(new StudioPreset(1, Library.Id, Playback.ClipId, Appearance, Bindings), JsonOptions);
    ...
    {
        var preset = JsonSerializer.Deserialize<StudioPreset>(File.ReadAllText(path), JsonOptions) ?? throw new InvalidDataException("Empty preset.");
    ...
    public void Apply(StudioPreset preset, string path)
    {
        ...
        if (preset.Appearance.CustomHead is not null && Library.Anatomy is not ("person" or "hound")) throw new InvalidDataException("Custom heads currently require a person or quadruped library.");
